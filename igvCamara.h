...
{  private:
      ...
      bool controlActivo = false; ///< Modo de movimiento interactivo activo
   ...
   public:
      ...
      // Nuevo: actualizar solo la razón de aspecto (se usa en reshape)
      void setAspecto(double a) { raspecto = a; }
      // Acceso rápido para cambiar entre proyecciones manteniendo parámetros
      void setTipo(tipoCamara t) { tipo = t; }
      tipoCamara getTipo() const { return tipo; }
      // Control de movimiento
      void toggleControl() { controlActivo = !controlActivo; }
      bool isControlActivo() const { return controlActivo; }
      void orbitY(double grados);    // órbita horizontal alrededor del origen (eje Y global)
      void pitch(double grados);     // cabeceo (rotación sobre eje X local / vector right)
      void yaw(double grados);       // rotación sobre eje Y local (up de la cámara)
      void adjustNear(double delta); // mover plano near
      void adjustFar(double delta);  // mover plano far
